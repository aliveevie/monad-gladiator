...
const RPS = '0x14A0559A19a724919D7CfEFF7BAFc42740d631F0';
const CF = '0xA2175f4774829dE597C1F5Ba6d52aA4700eC0Cd4';
...
function compile(name) {
  ...
  const input = { language: 'Solidity', sources,
    settings: { viaIR: true, optimizer: { enabled: true, runs: 200 }, evmVersion: 'prague',
      outputSelection: { '*': { '*': ['abi'] } } } };
...
async function playRPS(num) {
  ...
  stats.rps++;
  console.log(`  RPS #${num} (id:${matchId}): ${moveNames[moveA]} vs ${moveNames[moveB]} → ${result}`);
}
...
async function playCoinFlip(num) {
  ...
  stats.coinflip++;
  console.log(`  CoinFlip #${num} (id:${flipId}): resolved ✅`);
}

async function main() {
  console.log('╔═══════════════════════════════════════╗');
  console.log('║  MonadGladiator — Batch Match Engine  ║');
  console.log('╚═══════════════════════════════════════╝');
  ...
  // Play alternating RPS and CoinFlip
  for (let i = 1; i <= 15; i++) {
    try {
      if (i % 3 === 0) {
      ...
      console.log(`  Match ${i} error: ${e.message.slice(0, 80)}`);
      stats.errors++;
      if (e.message.includes('insufficient') || e.message.includes('Signer')) {
        console.log('  ⚠️ Out of funds, stopping');
        break;
      }
    }
  }
